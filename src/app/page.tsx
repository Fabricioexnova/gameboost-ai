...
import { Cpu, Zap, Activity, Thermometer, HardDrive, Wifi, Play, Pause, Settings, Shield, Eye, Monitor, Gamepad2, Sliders, Info, Power, Trash2, PauseCircle, Fan, Battery, Globe, RefreshCw, Moon, Sun, Languages, List, AlertCircle, CheckCircle, Clock, X, Star, TestTube } from 'lucide-react'
...
import { TelemetryBadge, TelemetryBadgeCompact } from '../components/TelemetryBadge'
...
export default function GameBoostAI() {
  ...
  const { envInfo, loading: electronLoading, isElectron, isAdmin, platform, isWindows } = useElectron()
  
  // Telemetry status
  const { status: telemetryStatus, refreshStatus: refreshTelemetryStatus } = useTelemetryStatus()
  ...
  const handleProfileSelect = async (profileName: string) => {
    ...
    const presetKey = profileKeyMap[profileName]
    
    if (!presetKey) {
      showNotification('error', `Preset '${profileName}' não encontrado`)
      return
    }
  ...
  // Handle individual optimizations with real optimizer integration
  const handleOptimizeCpuRam = async () => {
    if (initializing || turboRunning) {
      showNotification('warning', 'Aguarde a inicialização do engine ou fim do Turbo')
      return
    }
    
    showNotification('info', 'Liberando RAM...')
  ...
  const handleBoostGamePriority = async () => {
    ...
    showNotification('info', 'Pausando apps em segundo plano...')
  ...
  const handleTelemetryStatusChange = (status: 'real' | 'simulated') => {
    refreshTelemetryStatus()
    showNotification('info', `Telemetria alterada para: ${status === 'real' ? 'Real' : 'Simulada'}`)
  }
  ...
  const EnvironmentBadge = () => {
    if (electronLoading) return null
    
    if (!isElectron) {
      return <TelemetryBadgeCompact />
    }
    
    return (
      <div className="flex items-center space-x-3">
        <TelemetryBadgeCompact />
        <div className="flex items-center space-x-2 px-3 py-1 bg-gradient-to-r from-[#00ff88]/20 to-[#00cc6a]/20 border border-[#00ff88]/50 rounded-full">
          <Monitor className="w-4 h-4 text-[#00ff88]" />
          <span className="text-[#00ff88] text-sm font-medium">
            Desktop {isAdmin ? '(Admin)' : '(User)'}
          </span>
        </div>
      </div>
    )
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <ProfileCard
            name="Arc Raiders"
            description="Estabilidade em mapas grandes"
            icon={Shield}
            active={selectedProfile === 'Arc Raiders'}
            onClick={() => handleProfileSelect('Arc Raiders')}
            disabled={applying || initializing || turboRunning || cooldown > 0}
          />
          <ProfileCard
            name="Warzone"
            description="Latência mínima e FPS"
            icon={Zap}
            active={selectedProfile === 'Warzone'}
            onClick={() => handleProfileSelect('Warzone')}
            disabled={applying || initializing || turboRunning || cooldown > 0}
          />
        </div>
  ...
  const renderTelemetry = () => (
    <div className="space-y-8">
      <div className="text-center">
        <h2 className="text-3xl font-bold text-[#00ff88] mb-4">Configuração de Telemetria</h2>
        <p className="text-gray-400">Configure e teste as ferramentas de captura de dados</p>
      </div>

      {/* Telemetry Status Badge */}
      <div className="flex justify-center">
        <TelemetryBadge showDetails={true} />
      </div>

      {/* Telemetry Setup Component */}
      <TelemetrySetup onStatusChange={handleTelemetryStatusChange} />
    </div>
  )
